    ...
    InPin: InputPin + Wait,
{
    ...
    pub async fn poll_events(&mut self) -> ControllerEvent {
        let confirm = wait_for_falling_edge_debounced(&mut self.confirm, 50);
        let back = wait_for_falling_edge_debounced(&mut self.back, 50);
        let nav_next = wait_for_falling_edge_debounced(&mut self.nav_next, 50);
        let nav_prev = wait_for_falling_edge_debounced(&mut self.nav_prev, 50);
